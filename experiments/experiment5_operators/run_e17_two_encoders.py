...
E15 achieved StrategyQA 81.0% via ARTI-routed scoring (cosine + direct blend).
E16 showed that unfreezing the encoder destroys this breakthrough (SQA collapses
to 54.0%) because ARTI was trained on frozen-encoder embeddings and the unfrozen
encoder's output distribution shifts away from ARTI's training distribution.

The fix: decouple the ARTI routing signal from the encoder gradient flow by
providing **precomputed MiniLM embeddings** (ARTI's native input format)
separately from the unfrozen GPT-2 encoder.

Two-encoder data flow:
  - MiniLM (precomputed, frozen): feeds ARTI for routing — immune to drift
  - GPT-2 (live, unfrozen last 2 blocks): feeds downstream reasoning pipeline

No second encoder module in the model — MiniLM embeddings are precomputed at
data-loading time and passed through the batch as `st_embeddings`. When present,
ARTI uses them directly (bypassing the arti_adapter).

Conditions (2 x 3 seeds x 4 benchmarks):
  C1: Unfrozen GPT-2 + native MiniLM ARTI + 3 benchmarks (GSM8K+ARC+SQA)
  C2: Unfrozen GPT-2 + native MiniLM ARTI + 4 benchmarks (add FOLIO)

References:
  E15 C1 (frozen + routed): GSM8K 48.1%, SQA 81.0%, ARC 28.6%, FOLIO 33.5%
  E16 C1 (unfreeze 2 + adapter-routed): GSM8K ?, SQA 54.0% (collapsed)

Success Criteria:
  SC1: C1 StrategyQA >= 78% (preserve E15 breakthrough via native MiniLM)
  SC2: C1 GSM8K >= 48% (no regression)
  SC3: C1 ARC > 32% (break frozen-encoder ceiling)
  SC4: Router alpha differs by >0.1 between GSM8K and SQA (task-adaptive)
  SC5: C1 mean > E15 C1 mean (47.8%)

Usage:
    cd paper13_The_geometry_of_Machine_Reasoning
    python experiments/experiment5_operators/run_e17_two_encoders.py
"""
...
RESULTS_DIR = BASE_DIR / "results" / "e17_two_encoders"
...
CONDITIONS = {
    'C1_two_enc_3bench': {
        'description': 'Unfrozen GPT-2 + native MiniLM ARTI + 3 benchmarks (GSM8K+ARC+SQA)',
        'unfreeze_layers': 2,
        'encoder_lr': 1e-5,
    ...
    'C2_two_enc_4bench': {
        'description': 'Unfrozen GPT-2 + native MiniLM ARTI + 4 benchmarks (add FOLIO)',
        'unfreeze_layers': 2,
        'encoder_lr': 1e-5,
...
# ═══════════════════════════════════════════════════════════════════════════════
# Hybrid Dataset — Text tokens + precomputed MiniLM embeddings
# ═══════════════════════════════════════════════════════════════════════════════

class HybridMixedDomainDataset(TextBenchmarkDataset):
    """
    Extends TextBenchmarkDataset with precomputed MiniLM embeddings.

    Each example provides:
      - input_ids, attention_mask: GPT-2 tokens (for unfrozen encoder)
      - answer_input_ids, answer_attention_mask: GPT-2 tokenized answers
      - labels: correct answer index
      - valid_choices: bool mask for padded answer slots
      - st_embeddings: [384] precomputed MiniLM embedding (for ARTI)
    """
    ...
    def __init__(
        ...
        self._st_embeddings = st_embeddings

    def __getitem__(self, idx):
        item = super().__getitem__(idx)
        item['valid_choices'] = self._valid_choices[idx]
        item['st_embeddings'] = self._st_embeddings[idx]
        return item


def collate_hybrid(batch: List[Dict]) -> Dict[str, torch.Tensor]:
    """Collate function for HybridMixedDomainDataset."""
    result = {
    ...
    if 'st_embeddings' in batch[0]:
        result['st_embeddings'] = torch.stack([b['st_embeddings'] for b in batch])
    return result


def _encode_questions_minilm(questions: List[str]) -> torch.Tensor:
    """Encode questions with MiniLM sentence-transformer. Returns [N, 384] CPU tensor."""
    from sentence_transformers import SentenceTransformer
    st_model = SentenceTransformer('all-MiniLM-L6-v2')
    with torch.no_grad():
        embeddings = st_model.encode(
            questions, batch_size=128,
...
def load_hybrid_training_data(
    ...
    seed: int,
    max_length: int = 128,
) -> Tuple[HybridMixedDomainDataset, HybridMixedDomainDataset]:
    """Load tokenized text training data + precomputed MiniLM embeddings."""
    logger.info(f"  Loading hybrid training data for: {train_benchmarks}")

    # Step 1: Load raw questions for MiniLM encoding
    all_raw_questions = []
    bench_sizes = {}

    for bench_name in train_benchmarks:
        loader_fn = LOADERS[bench_name]
        kwargs = {'split': 'train'}
        ...
        bench_sizes[bench_name] = len(questions)
        logger.info(f"    {bench_name}: {len(questions)} raw questions loaded")

    # Step 2: Encode all questions with MiniLM (one batch, efficient)
    logger.info(f"  Encoding {len(all_raw_questions)} questions with MiniLM...")
    all_st_embeddings = _encode_questions_minilm(all_raw_questions)
    logger.info(f"  MiniLM encoding complete: {all_st_embeddings.shape}")

    # Step 3: Load tokenized datasets (for GPT-2)
    datasets = {}
    for bench_name in train_benchmarks:
        ...
        logger.info(f"    {bench_name}: {len(ds)} tokenized, "
                     f"n_choices={ds.answer_input_ids.shape[1]}")

    # Step 4: Pad to max_choices and combine
    max_choices = max(ds.answer_input_ids.shape[1] for ds in datasets.values())
    L = next(iter(datasets.values())).question_input_ids.shape[1]
    ...
    for name in train_benchmarks:
        ...
        # MiniLM embeddings for this benchmark's questions
        all_st.append(all_st_embeddings[st_offset:st_offset + n])
        st_offset += n
    ...
    st_embs = torch.cat(all_st, dim=0)

    N = len(labels)
    logger.info(f"  Combined: {N} examples, max_choices={max_choices}, "
                f"st_embeddings={st_embs.shape}")
    ...
    train_ds = HybridMixedDomainDataset(
        ...
        st_embeddings=st_embs[train_idx],
    )
    val_ds = HybridMixedDomainDataset(
...
def train_condition(cond_name: str, condition: dict, seed: int) -> COFRN:
    ...
    # Load hybrid training data (GPT-2 tokens + MiniLM embeddings)
    train_ds, val_ds = load_hybrid_training_data(
...
def evaluate_full(model: COFRN, seed: int) -> dict:
    ...
    Uses text-based evaluation with precomputed MiniLM embeddings for ARTI.
    """
    results = {}
    ...
    for bench, split, n_choices in BENCHMARKS:
        ...
        # Load raw questions for MiniLM encoding
        loader_fn = LOADERS[bench]
        kwargs = {'split': split}
        if 'seed' in inspect.signature(loader_fn).parameters:
            kwargs['seed'] = seed
        questions, _, _ = loader_fn(**kwargs)

        # Encode questions with MiniLM
        st_embeddings = _encode_questions_minilm(questions)
        ...
        for start in range(0, n, batch_size):
            ...
            out = model(
                ...
                st_embeddings=st_embeddings[start:end].to(DEVICE),
            )
...
def main():
    t0 = time.time()
    logger.info("=" * 70)
    logger.info("  E17 TWO-ENCODER: NATIVE MiniLM FOR ARTI ROUTING")
    ...
    for bench, _, n_choices in BENCHMARKS:
        ...
        logger.info(f"\n  {bench} (random={1/n_choices:.1%}, E15_C1={e15_acc:.1%}):")
    ...
    # ─── Router alpha per-benchmark analysis ─────────────────────────
    logger.info(f"\n{'='*70}")
    logger.info("  ROUTER ALPHA PER-BENCHMARK (task-adaptivity check)")
    logger.info(f"{'='*70}")
    ...
    for cond_name in CONDITIONS:
        ...
        for bench, _, _ in BENCHMARKS:
            a = aggregate[cond_name][bench]
            if 'mean_router_alpha' in a:
                bench_alphas[bench] = a['mean_router_alpha']
                logger.info(f"    {bench:15s}: alpha={a['mean_router_alpha']:.4f} "
                             f"+/- {a.get('std_router_alpha', 0):.4f}")

        if 'gsm8k' in bench_alphas and 'strategyqa' in bench_alphas:
            delta_alpha = abs(bench_alphas['gsm8k'] - bench_alphas['strategyqa'])
            logger.info(f"    |alpha(GSM8K) - alpha(SQA)| = {delta_alpha:.4f} "
                         f"(target > 0.1: {'PASS' if delta_alpha > 0.1 else 'FAIL'})")
    ...
    c1 = aggregate.get('C1_two_enc_3bench', {})
    c2 = aggregate.get('C2_two_enc_4bench', {})

    # SC1: C1 StrategyQA >= 78%
    c1_sqa = c1.get('strategyqa', {}).get('mean_acc', 0)
    sc1 = c1_sqa >= 0.78
    logger.info(f"  SC1: C1 StrategyQA >= 78%: {c1_sqa:.1%} -> {'PASS' if sc1 else 'FAIL'}")

    # SC2: C1 GSM8K >= 48%
    c1_gsm = c1.get('gsm8k', {}).get('mean_acc', 0)
    sc2 = c1_gsm >= 0.48
    logger.info(f"  SC2: C1 GSM8K >= 48%: {c1_gsm:.1%} -> {'PASS' if sc2 else 'FAIL'}")
    ...
    # SC4: Router alpha differs by >0.1 between GSM8K and SQA
    c1_alpha_gsm = c1.get('gsm8k', {}).get('mean_router_alpha', 0)
    c1_alpha_sqa = c1.get('strategyqa', {}).get('mean_router_alpha', 0)
    delta_alpha = abs(c1_alpha_gsm - c1_alpha_sqa)
    sc4 = delta_alpha > 0.1
    logger.info(
        f"  SC4: |alpha(GSM8K) - alpha(SQA)| > 0.1: "
        f"|{c1_alpha_gsm:.3f} - {c1_alpha_sqa:.3f}| = {delta_alpha:.3f} "
    ...
    output = {
        ...
        'description': (
            'E17: Two-encoder architecture — native MiniLM for ARTI routing, '
            'unfrozen GPT-2 for downstream reasoning. Decouples ARTI from '
            'encoder gradient flow to prevent distribution shift.'
        ),
        ...
        'aggregate': aggregate,
        'success_criteria': {
            'SC1_sqa_preserved': sc1,
            'SC2_gsm8k_no_regression': sc2,
            'SC3_arc_above_32': sc3,
            'SC4_alpha_task_adaptive': sc4,
    ...
    out_path = RESULTS_DIR / "e17_results.json"
