...
E17 falsified the ARTI distribution shift hypothesis: native MiniLM embeddings
produced identical results to E16 (SQA 54.0%±0.0%). The root cause is answer
embedding destabilization: when the shared GPT-2 encoder is unfrozen, generic
labels (Yes/No) become nearly identical in embedding space.

E18 tests: does freezing the answer encoding path (torch.no_grad + detach)
while keeping the question path unfrozen preserve E15's StrategyQA 81%?

Architecture:
  Question path:  input_ids → encoder(unfrozen last 2) → s0 → factorization → reasoning
  Answer path:    answer_ids → encoder(FROZEN via no_grad+detach) → answer_proj → answer_enc
  ARTI routing:   adapter(s0) → ARTI(frozen) → arti_probs → router → alpha

Key change: COFRNConfig(freeze_answer_encoder=True) — one flag in model.py
that wraps encode_answers_from_text() in torch.no_grad() + detach().

Conditions (2 × 3 seeds × 4 benchmarks):
  C1: Unfreeze Q + freeze A + ARTI-routed + 3 benchmarks (GSM8K + ARC + SQA)
  C2: Unfreeze Q + freeze A + ARTI-routed + 4 benchmarks (add FOLIO)

References:
  E15 C1 (frozen + routed): GSM8K 48.1%, SQA 81.0%, ARC 28.6%, FOLIO 33.5%
  E16 C1 (unfrozen + routed): GSM8K 49.8%, SQA 54.0%, ARC 28.2%, FOLIO 33.8%
  E17 C1 (two-encoder):       GSM8K 49.2%, SQA 54.0%, ARC 27.4%, FOLIO 33.7%

Success Criteria:
  SC1: C1 StrategyQA >= 78% (preserve E15 breakthrough)
  ...
  SC5: Best condition mean > E15 C1 mean (47.8%)

Usage:
    cd paper13_The_geometry_of_Machine_Reasoning
    python experiments/experiment5_operators/run_e18_freeze_answers.py
"""
...
RESULTS_DIR = BASE_DIR / "results" / "e18_freeze_answers"
...
E16_C1_BASELINES = {
    'gsm8k':         {'mean': 0.498, 'std': 0.018},
    'arc_challenge': {'mean': 0.282, 'std': 0.012},
    'strategyqa':    {'mean': 0.540, 'std': 0.000},
    'folio':         {'mean': 0.338, 'std': 0.028},
}

E17_C1_BASELINES = {
    'gsm8k':         {'mean': 0.492, 'std': 0.015},
    'arc_challenge': {'mean': 0.274, 'std': 0.009},
    'strategyqa':    {'mean': 0.540, 'std': 0.000},
    'folio':         {'mean': 0.337, 'std': 0.027},
}
...
CONDITIONS = {
    'C1_freeze_ans_3bench': {
        'description': 'Unfreeze Q + freeze A + ARTI-routed + 3 benchmarks (GSM8K+ARC+SQA)',
        'unfreeze_layers': 2,
        'encoder_lr': 1e-5,
    ...
    'C2_freeze_ans_4bench': {
        'description': 'Unfreeze Q + freeze A + ARTI-routed + 4 benchmarks (add FOLIO)',
        'unfreeze_layers': 2,
        'encoder_lr': 1e-5,
...
# (Identical to E16 — freeze logic is in model.py, not the data pipeline)
# ═══════════════════════════════════════════════════════════════════════════════
...
def build_model(condition: dict, seed: int) -> COFRN:
    """Build COFRN with unfrozen question encoder + frozen answer encoder + ARTI routing."""
    config = COFRNConfig(
        ...
        # E18: Freeze answer encoding path
        freeze_answer_encoder=True,
        # E15: ARTI routing
...
def train_condition(cond_name: str, condition: dict, seed: int) -> COFRN:
    ...
    logger.info(f"  freeze_answer_encoder={model.config.freeze_answer_encoder}")
...
def _verify_gradient_flow(model: COFRN):
    """Verify encoder layers have non-zero grads (from question path only)."""
    ...
    aproj_grad_params = sum(
        1 for p in model.answer_proj.parameters()
        if p.requires_grad and p.grad is not None and p.grad.abs().sum() > 0
    )
    aproj_trainable = sum(
        1 for p in model.answer_proj.parameters()
    ...
    logger.info(
        f"  Gradient flow: encoder={encoder_grad_params}/{encoder_trainable} "
        f"answer_proj={aproj_grad_params}/{aproj_trainable} "
        f"(freeze_answer_encoder={model.config.freeze_answer_encoder})"
    )
...
def main():
    t0 = time.time()
    logger.info("=" * 70)
    logger.info("  E18 FREEZE ANSWER ENCODER + UNFREEZE QUESTION ENCODER")
    ...
    for bench, _, n_choices in BENCHMARKS:
        ...
        e16_acc = E16_C1_BASELINES[bench]['mean']
        e17_acc = E17_C1_BASELINES[bench]['mean']
        logger.info(f"\n  {bench} (random={1/n_choices:.1%}, "
                     f"E15={e15_acc:.1%}, E16={e16_acc:.1%}, E17={e17_acc:.1%}):")
        header = (f"  {'Condition':35s} {'Acc':>12s} "
                  f"{'Δ vs E15':>8s} {'Δ vs E16':>8s} "
                  f"{'Ent.R':>8s} {'Alpha':>8s}")
        logger.info(header)
        logger.info(f"  {'-'*86}")
    ...
    c1 = aggregate.get('C1_freeze_ans_3bench', {})
    c2 = aggregate.get('C2_freeze_ans_4bench', {})
    ...
    output = {
        ...
        'description': (
            'E18: Freeze answer encoder path (torch.no_grad+detach) while '
            'unfreezing question encoder (last 2 GPT-2 blocks). Tests whether '
            'answer embedding destabilization is caused by answer-path gradients '
            'vs encoder weight changes from question-path gradients.'
        ),
        'hypothesis': (
            'Freezing the answer encoding path preserves E15 StrategyQA 81% '
            'by preventing answer embedding destabilization, while unfrozen '
            'question path allows potential ARC/FOLIO improvements.'
        ),
        ...
        'arti_checkpoint': ARTI_CHECKPOINT,
        'baselines': {
            'e15_c1': E15_C1_BASELINES,
            'e16_c1': E16_C1_BASELINES,
            'e17_c1': E17_C1_BASELINES,
        },
    ...
    out_path = RESULTS_DIR / "e18_results.json"
